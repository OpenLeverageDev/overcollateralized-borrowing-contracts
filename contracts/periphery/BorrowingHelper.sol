...
contract BorrowingHelper {
    ...
    function collateralRatios(IOPBorrowing borrowing, uint16[] calldata marketIds, address[] calldata borrowers, bool[] calldata collateralIndexes) external view returns (uint[] memory results){
    ...
    function getBorrowingStat(IOPBorrowing borrowing, address borrower, uint16 marketId, bool collateralIndex) external returns (BorrowingStatVars memory){
        ...
        bytes memory dexData;
        {
            ...
            (,,,, result.lastUpdateTime) = OPBorrowingStorage(address(borrowing)).dexAgg().getPriceCAvgPriceHAvgPrice(collateralToken, borrowToken, 60, dexData);
